...
//        index     = new LinHashMap <> (KeyType.class, Comparable [].class);
    ...
    {
        out.println ("RA> " + name + ".minus (" + table2.name + ")" + tuples.size() + "-" + attribute.length);
        if (! compatible (table2)) return null;
        
        List <Comparable []> rows = new ArrayList <> ();
        //Comparable [] x = tuples.get(0);
        /*out.printf ("%15s", tuples.get(0));
        out.printf ("%15s", tuples.get(1));
        out.printf ("%15s", tuples.get(2));
        out.printf ("%15s", tuples.get(3));
        Comparable[] tup1 = tuples.get(0);
        Comparable t= tup1[1];
        out.printf ("%15s*", t);
        out.print ("\n\n");*/
        //  T O   B E   I M P L E M E N T E D 
        ...
        for (Comparable [] tup : tuples) {
        	for (Comparable [] tabletup : table2.tuples) {
        		if(tup[0].equals(tabletup[0]) && tup[1] == tabletup[1])
        			notexist = false;
        	}
        	if(notexist) rows.add(tup);
        	notexist = true;
        }
